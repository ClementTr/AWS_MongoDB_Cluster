...
def extractFileList(rawHTML):
    regexZIP = re.compile(">(201712[0-9]{2}\.export\.CSV\.zip)<\/A>") #We select only the file we want (date selection)
...
def insertDF(df):
    # MONGO
    client = MongoClient('mongodb://172.31.31.145:27016,172.31.29.10:27018,172.31.22.59:27018/?replicaSet=rs0')
    db = client.gdelt
    events = db.events

...
ip_public_master = 'mongodb://localhost:27016,localhost:27018,localhost:27018/?replicaSet=rs0'
...
def clusterRequest(actor2):
    os.system("ssh " + ip + " python mongoRequest.py " + Actor1 + " " +
                Actor2 + " " + Date1 + " " + Date2 + " >> frequencies.csv")

def initClusterRequest():
    os.system("ssh " + ip + " echo 'Country,Frequency' > frequencies.csv")


def distantRequestMongo(Actor1Code, Date1, Date2):
    actor2list = getCountrieslist()
    ...
    p = Pool(5)
    p.map(clusterRequest, actor2list)
